// Define global variables to avoid "not defined" errors during bundling
window.TREX_STRICT_MODE = false;
window.DEBUG = 1;

// Mocking EditorJSLoader since we are bypassing it
window.EditorJSLoader = {
    getJSBasePath: () => './src/legacy/js/',
    loadModule: () => {},
    ready: (fn) => {
        // We will call this manually or ensure it runs after everything is loaded
        if (window.Editor) fn(window.Editor);
    },
    finish: () => {
        console.log('Legacy loader finished');
    }
};
...
const loadLegacyScripts = async () => {
    // Header
    window.EditorJSLoader.readyState = 'loading';
    
    // Core Files
    for (const file of CORE_FILES) {
        try {
            // Sequential dynamic import to mimic script loading order
            await import(`./legacy/js/${file}`);
        } catch (e) {
            console.error(`Failed to load legacy script: ${file}`, e);
        }
    }
    
    // Footer
    window.EditorJSLoader.readyState = 'complete';
    if (window.EditorJSLoader.finish) window.EditorJSLoader.finish();
    
    console.log('DaumEditor legacy scripts loaded successfully');
};
